...
const limiter = rateLimit({
  ...
  skip: (req) => {
    // Skip rate limiting for development and OPTIONS requests
...
// Apply rate limiting with more lenient settings for development
if (process.env.NODE_ENV === 'development') {
  // More lenient rate limiting for development
  const devLimiter = rateLimit({
    windowMs: 15 * 60 * 1000, // 15 minutes
    max: 10000, // 10000 requests per 15 minutes for development
    message: 'Too many requests from this IP, please try again later.',
...
// Apply specific rate limiting with different settings for development vs production
if (process.env.NODE_ENV === 'development') {
  // More lenient rate limiting for development
  const devAuthLimiter = rateLimit({
    windowMs: 15 * 60 * 1000,
    max: 50, // 50 login attempts per 15 minutes for development
    message: 'Too many login attempts, please try again later.',
    standardHeaders: true,
    legacyHeaders: false,
  ...
  const devUploadLimiter = rateLimit({
    windowMs: 60 * 60 * 1000,
    max: 100, // 100 uploads per hour for development
    message: 'Too many file uploads, please try again later.',
    standardHeaders: true,
    legacyHeaders: false,
    skip: (req) => req.method === 'OPTIONS'
  ...
  app.use('/api/upload', devUploadLimiter);
  // Skip specific rate limiting for customers and reports in development
} else {
  // Production rate limiting
