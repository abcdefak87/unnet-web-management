...
router.post('/login', [
  ...
  try {
    ...
    const { refreshToken, tokenId } = tokenManager.generateRefreshToken(user.id);

      // Persist hashed refresh token for server-side verification
      try {
        const saltRounds = parseInt(process.env.BCRYPT_SALT_ROUNDS) || 14;
...
router.post('/refresh', async (req, res) => {
  try {
    ...
    let rotated;
    try {
      rotated = tokenManager.rotateRefreshToken(decoded.tokenId, user.id);
    } catch (rotationError) {
      return res.status(401).json({ error: 'Refresh token tidak valid' });
    }
    ...
    try {
      const saltRounds = parseInt(process.env.BCRYPT_SALT_ROUNDS) || 14;
      const newHash = await bcrypt.hash(rotated.refreshToken, saltRounds);
...
router.post('/logout', authenticateToken, async (req, res) => {
  try {
    ...
    try { tokenManager.revokeUserTokens(req.user.id); } catch (e) {}
    await prisma.user.update({
      where: { id: req.user.id },
